...
use Mcs\WpModels\ProvinceFieldValues;
use Mcs\WpModels\Provinces;

class testWpProvinceFieldValuesController extends WP_Test_REST_Controller_Testcase {
	...
	/**
	 * @var ProvinceFieldValues
	 */
	protected $provinceFieldValue;
	...
	public function setUp() {
		...
		$this->provinceFieldValue = ProvinceFieldValues::create( [
	...
	public function tearDown() {
		$this->provinceFieldValue->delete();
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/mcs/v1/ProvinceFieldValues', $routes );
		$this->assertArrayHasKey( '/mcs/v1/ProvinceFieldValues/(?P<id>[\d]+)', $routes );
	}
	...
	public function test_get_items() {
		...
		$request = new WP_REST_Request( 'GET', '/mcs/v1/ProvinceFieldValues' );
		...
		$data      = $all_data[0];
		$modelData = ProvinceFieldValues::findById( $data['id'] );
	...
	public function test_get_item() {
		...
		$request  = new WP_REST_Request( 'GET', sprintf( '/mcs/v1/ProvinceFieldValues/%d', $this->provinceFieldValue->id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_create_item() {
		...
		$request = new WP_REST_Request( 'POST', '/mcs/v1/ProvinceFieldValues' );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
		$params = $this->set_model_data();
		$request->set_body_params( $params );
		$response = rest_get_server()->dispatch( $request );
		$this->check_create_model_response( $response );
		ProvinceFieldValues::findById( $response->get_data()['id'] )->delete();
	}

	public function test_update_item() {
		wp_set_current_user( self::$user );
		$request = new WP_REST_Request( 'PUT', sprintf( '/mcs/v1/ProvinceFieldValues/%d', $this->provinceFieldValue->id ) );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
		$params = $this->set_model_data();
		$request->set_body_params( $params );
		$response = rest_get_server()->dispatch( $request );
		...
		$this->provinceFieldValue = ProvinceFieldValues::findById( $this->provinceFieldValue->id );
		$this->check_model_data( $this->provinceFieldValue, $params );
	}
	...
	public function test_delete_item() {
		wp_set_current_user( self::$user );
		$request  = new WP_REST_Request( 'DELETE', sprintf( '/mcs/v1/ProvinceFieldValues/%d', $this->provinceFieldValue->id ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
		$model = ProvinceFieldValues::findById( $this->provinceFieldValue->id );
		$this->assertNull( $model );
//		$this->assertEquals( 404, $model->get_error_code() );
		$this->provinceFieldValue = ProvinceFieldValues::create( $this->set_model_data() );
	}
	...
	protected function check_get_field_response( $response ) {
		...
		$data     = $response->get_data();
		$cityData = ProvinceFieldValues::findById( $data['id'] );
		$this->check_model_data( $cityData, $data );
	}

	protected function check_model_data( ProvinceFieldValues $model, array $data ) {
		if ( isset( $data['id'] ) ) {
	...
	protected function check_create_model_response( WP_REST_Response $response ) {
		...
		$model = ProvinceFieldValues::findById( $data['id'] );
